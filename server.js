    ...
    if (req.isAuthenticated()) {
      ...
      if (dbquery) {
        dbresult = await db.query(
          `SELECT * FROM bookreviewlist WHERE email = $1 ${dbquery}`,
          [loggedinemail],
        );
      } else {
        dbresult = await db.query(
          `SELECT * FROM bookreviewlist WHERE email = $1`,
          [loggedinemail],
        );
      }
...
app.get("/editbook/:id", async (req, res) => {
  try {
    if (req.isAuthenticated()) {
      ...
      const editdbresult = await db.query(
        `SELECT * FROM bookreviewlist WHERE id = $1`,
        [bookid],
      );
      console.log(
        "The book title db result for edit is: ",
        editdbresult.rows[0].title,
      );
  ...
  if (req.isAuthenticated()) {
    ...
    try {
      const response = await axios.get(
        `https://openlibrary.org/search.json?title=${newtitlewords}`,
      );

      var isbn13;

      if (response.data.docs[0].ia.find((item) => item.startsWith("isbn"))) {
        isbn13 = response.data.docs[0].ia
          .find((item) => item.startsWith("isbn"))
          .slice(5);
    ...
    if (isbn13) {
      ...
      try {
        const time = new Date();
        await db.query(
          "INSERT INTO bookreviewlist(title, author, rating, summary, isbn, date, email) VALUES ($1, $2, $3, $4, $5, $6, $7)",
          [
            newbooktitle,
            newbookauthor,
            newbookrating,
        ...
        console.error(
          "error inserting into database with isbn13 present is: " + error,
        );
      }
    } else {
      ...
      try {
        ...
        console.error(
          "error inserting into database without isbn13 present is: " + error,
        );
      }
    }
    res.redirect("/");
...
app.post("/posteditedbook", async (req, res) => {
  try {
    if (req.isAuthenticated()) {
      ...
      await db.query(
        "UPDATE bookreviewlist SET title=$1, author=$2, rating=$3, summary=$4, date=$5 WHERE id=$6",
        [
          updatedbooktitle,
          updatedbookauthor,
          updatedbookrating,
